...
usage: $0 <options>
  ...
  Optional options:
     --lib-dir=DIR               path to install doris home [/usr/lib/doris]
...
LIB_DIR=${LIB_DIR:-/doris}
ETC_DINKY=${ETC_DINKY:-/etc/doris}

install -d -m 0755 $PREFIX/$LIB_DIR
install -d -m 0755 $PREFIX/$LIB_DIR/bin
install -d -m 0755 $PREFIX/$LIB_DIR/deploy
install -d -m 0755 $PREFIX/$LIB_DIR/doris-loader
...
install -d -m 0755 $PREFIX/var/log/doris
install -d -m 0755 $PREFIX/var/run/doris

cp -ra $BUILD_DIR/bin/*.sh ${PREFIX}/${LIB_DIR}/bin/
cp -ra $BUILD_DIR/deploy/* ${PREFIX}/${LIB_DIR}/deploy/
cp -ra $BUILD_DIR/doris-loader/* ${PREFIX}/${LIB_DIR}/doris-loader/
...
ln -s /var/log/doris $PREFIX/$LIB_DIR/logs
ln -s /var/run/doris $PREFIX/$LIB_DIR/pid
