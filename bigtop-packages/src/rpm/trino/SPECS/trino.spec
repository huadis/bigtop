...
%define lib_trino %{parent_dir}/%{trino_name}
%define lib_trino_cli %{parent_dir}/%{trino_cli_name}
...
%define trino_current_version 414
...
%package -n %{trino_pkg_name}-server
Summary: trino Server
Group: Development/Libraries
BuildArch: noarch
Requires: %{name} = %{version}-%{release}

%description -n %{trino_pkg_name}-server
Server for trino

%package -n %{trino_pkg_name}-cli
Summary: trino CLI
Group: Development/Libraries
BuildArch: noarch
Requires: %{name} = %{version}-%{release}

%description -n %{trino_pkg_name}-cli
CLI for trino
...
bash %{SOURCE2} \
          ...
          --build-cli-dir=build/trino-cli \
          --lib-dir=%{lib_trino} \
          --cli-dir=%{lib_trino_cli} \
