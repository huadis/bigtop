...
Source0: %{hue_name}-%{hue_base_version}.tar.gz
Source1: %{hue_name}.init
Source2: %{hue_name}.init.suse
...
%prep
%setup -n %{hue_name}-release-%{hue_base_version}
...
%else
orig_init_file=$RPM_SOURCE_DIR/%{hue_name}.init
%endif
...
if [ !  -e "/etc/hue/conf" ]; then
      rm -f /etc/hue/conf
      mkdir -p /etc/hue/conf
      cp -rp /etc/hue/conf.empty/* /etc/hue/conf
fi
ln -s /var/log/hue/ %{crh_dir}/hue/logs
ln -s /etc/hue/conf %{crh_dir}/hue/conf
chmod 777 %{crh_dir}/hue/
...
%preun  -n %{name}-server
if [ $1 = 0 ] ; then
        service %{hue_name} stop > /dev/null 2>&1
        chkconfig --del %{hue_name}
fi
%postun  -n %{name}-server
if [ $1 -ge 1 ]; then
        service %{hue_name} condrestart >/dev/null 2>&1
fi
#### HUE-IMPALA PLUGIN ######
