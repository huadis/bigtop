...
%define tomcat_conf_sqoop %{_sysconfdir}/sqoop/tomcat-conf
...
License: ASL 2.0
Source0: sqoop-%{sqoop_base_version}.tar.gz
...
bash %{SOURCE2} \
          ...
          --initd-dir=%{initd_dir} \
          --dist-dir=dist/target/sqoop-%{sqoop_base_version}
...
getent passwd sqoop >/dev/null || useradd -c "Sqoop 2 User" -s /sbin/nologin -g sqoop -r -d /var/lib/sqoop sqoop 2> /dev/null || :
...
%{alternatives_cmd} --install %{tomcat_conf_sqoop} sqoop-tomcat-conf %{tomcat_conf_sqoop_dist} 30
...
if [ "$1" = "0" ] ; then
  ...
  %{alternatives_cmd} --remove sqoop-tomcat-conf %{tomcat_conf_sqoop_dist} || :
fi
